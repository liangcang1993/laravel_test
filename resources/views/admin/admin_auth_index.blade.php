...
<div class="container">
    ...
    <div class="page-header">
            ...
            <a class="btn btn-white btn-info btn-bold" href="/admin/admin_auth/create">
...
<script>
    ...
    function add(id) {
        // alert("add 方法 , id = " + id);
        $('#hidepid').val(id);
        $('#exampleModalLabel').html('添加权限');
        $('#recipient-name').val('');
        $('#recipient-controller').val('');
        $('#recipient-action').val('');
        $('#recipient-uri').val('');
        $('#recipient-weight').val('');
        $('#recipient-status').val(0);
        $("#myForm").attr("action", '{{ URL('admin/authAdd') }}');
    }
    function del(id) {
        // alert("del 方法 , id = " + id);
        $.ajax({
            type: "get",
            url: '{{url('/admin/authDel')}}',
            dataType: "json",
            data: {
                'id':id
            },
    ...
    function update(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#exampleModalLabel').html('修改权限');
                $('#hidepid').val(data.pid);
                $('#hideid').val(data.id);
                $('#recipient-name').val(data.name);
                $('#recipient-controller').val(data.controller);
                $('#recipient-action').val(data.action);
                $('#recipient-uri').val(data.uri);
                $('#recipient-weight').val(data.weight);
                $('#recipient-status').val(data.status);
                $("#myForm").attr("action", '{{ URL('admin/authUpdate') }}');
